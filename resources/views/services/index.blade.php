        ...
        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
            <div class="space-y-3">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h1 class="text-2xl md:text-3xl font-bold text-gray-900">{{ __('Patients') }}</h1>
                        <p class="text-gray-500 mt-1">
                            {{ __('Total :') }} <span class="font-bold text-gray-900">{{ $patients->count() }}</span> {{ __('patient(s)') }}
                        </p>
                    </div>
                </div>
            </div>
    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            ...
            <div class="hidden md:flex items-center gap-3 text-sm text-gray-600 bg-gray-50 px-4 py-2.5 rounded-lg border border-gray-200">
                ...
                <span class="font-bold text-gray-900" id="visibleCount">{{ $patients->count() }}</span>
                <span>{{ __('sur') }}</span>
                <span class="font-bold text-gray-900">{{ $patients->count() }}</span>
            </div>
        </div>
        ...
        @if($patients->isEmpty())
            <div class="text-center py-16 lg:py-20">
                <div class="inline-flex p-6 rounded-2xl bg-blue-50 mb-6">
                    <i class="fas fa-users text-blue-400 text-5xl"></i>
                </div>
                <h3 class="text-xl md:text-2xl font-semibold text-gray-800 mb-3">{{ __('Aucun patient trouvé') }}</h3>
                <p class="text-gray-500 max-w-md mx-auto mb-8">{{ __('Ajoutez votre premier patient pour commencer.') }}</p>
                <button onclick="showAjouteModal()"
                    ...
                    <i class="fas fa-plus"></i>{{ __('Ajouter le premier patient') }}
                </button>
            </div>
        @else
            <div class="space-y-3" id="patientsTable">
                @foreach ($patients as $index => $p)
                    <div class="patient-row group flex items-center justify-between p-4 bg-white rounded-xl border border-gray-200 hover:border-gray-300 shadow-sm hover:shadow-lg transition-all duration-300 cursor-pointer"
                         onclick="window.location='{{ route('patient.show', $p->id) }}'"
        ...
        <div class="relative w-full max-w-md bg-white rounded-2xl shadow-2xl border border-gray-200 overflow-hidden">
            ...
            <div class="p-6">
                <div class="flex items-start gap-4 mb-5">
                    ...
                    <div class="flex-1">
                        <h3 class="text-xl font-bold text-gray-900">{{ __('Ajouter un patient') }}</h3>
                        <p class="text-gray-500 text-sm mt-1">{{ __('Remplissez les informations du patient') }}</p>
                    </div>
                ...
                <form action="{{ route('patient.store') }}" method="POST" class="space-y-4">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="text" name="name" required placeholder="{{ __('Jean Dupont') }}"
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="email" name="email" required placeholder="{{ __('jean@exemple.com') }}"
                                class="w-full pl-12 pr-4 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-400/30 focus:border-blue-400 transition-all bg-gray-50 focus:bg-white">
                        </div>
                    </div>
                    <div>
        ...
        <div class="relative w-full max-w-md bg-white rounded-2xl shadow-2xl border border-gray-200 overflow-hidden">
            ...
            <div class="p-6">
                <div class="flex items-start gap-4 mb-4">
                    ...
                    <div class="flex-1">
                        <h3 class="text-xl font-bold text-gray-900">{{ __('Supprimer le patient') }}</h3>
                ...
                <div class="mb-6 p-4 bg-red-50 rounded-xl border border-red-200">
                    <p class="text-gray-700 text-sm leading-relaxed">{{ __('Êtes-vous sûr de vouloir supprimer ce patient ?') }}</p>
    ...
    <script>
        ...
        document.querySelectorAll('.action-menu-btn').forEach(btn => {
            btn.addEventListener('click', function(e) {
                ...
                const patientPhone = row?.dataset.patientPhone || '';
                
                currentPatientId = patientId;
                
                // Update menu links
                document.getElementById('menuView').href = `/patient/${patientId}`;
                document.getElementById('menuEdit').href = `/patient/${patientId}/edit`;
        ...
        document.getElementById('menuDelete').addEventListener('click', function(e) {
            ...
            floatingMenu.classList.add('hidden');
            if (currentPatientId) {
                document.getElementById('deleteForm').action = `/patient/${currentPatientId}`;
        ...
        searchInput?.addEventListener('keyup', function () {
            ...
            let count = 0;
            document.querySelectorAll('#patientsTable .patient-row').forEach(row => {
