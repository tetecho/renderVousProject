...
<x-app-layout>
    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            <div class="flex-1 max-w-2xl">
                <div class="relative group">
                    <i
                        class="fas fa-search absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 text-sm transition-colors duration-200 group-focus-within:text-blue-500"></i>
                    <input type="text" id="searchInput" placeholder="{{ __('Rechercher un médecin...') }}"
            ...
            <div
                class="hidden md:flex items-center gap-3 text-sm text-gray-600 bg-gray-50 px-4 py-2.5 rounded-lg border border-gray-200">
        ...
        @else
            <div class="space-y-3" id="patientsTable">
                @foreach ($medecins as $index => $p)
                    <div class="patient-row group flex items-center justify-between p-4 bg-white rounded-xl border border-gray-200 hover:border-gray-300 shadow-sm hover:shadow-lg transition-all duration-300 cursor-pointer"
                        onclick="window.location='{{ route('medecin.show', $p->id) }}'" data-patient-id="{{ $p->id }}"
                        data-patient-name="{{ $p->name }}" data-patient-email="{{ $p->email }}"
                        data-patient-phone="{{ $p->phone }}">

                        {{-- Left: avatar + info --}}
                        <div class="flex items-center gap-4 flex-1 min-w-0">
                            <div
                                class="w-11 h-11 gradient-primary rounded-full flex items-center justify-center shadow-md shrink-0">
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 mb-1">
                                    ...
                                    <span
                                        class="px-2 py-0.5 text-xs font-medium rounded-full bg-blue-50 text-blue-600 border border-blue-100 shadow-sm shrink-0">
                        ...
                        <div class="flex items-center gap-2 shrink-0 ml-4" onclick="event.stopPropagation()">
                            <button type="button"
                                ...
                                data-patient-id="{{ $p->id }}" aria-label="{{ __('Actions') }}">
    ...
    {{-- Floating action menu — rendered at body level, positioned via JS --}}
    <div id="floatingMenu" class="fixed hidden bg-white rounded-xl border border-gray-200 py-2"
        style="z-index:9999; min-width:220px; box-shadow: 0 10px 40px rgba(0,0,0,0.12), 0 0 0 1px rgba(0,0,0,0.05);">

        <a id="menuShow" href="#"
            class="group flex items-center gap-3 px-4 py-3 hover:bg-gray-50 transition-all duration-200">
        ...
        <button id="menuDelete" type="button"
            class="group flex items-center gap-3 px-4 py-3 hover:bg-red-50 transition-all duration-200 w-full text-left">
    ...
    {{-- MODAL — Ajouter --}}
    <div id="formAjoute" class="fixed inset-0 hidden items-center justify-center p-4 bg-black/40 backdrop-blur-sm"
        style="z-index:10000;">
        <div class="relative w-full max-w-md bg-white rounded-2xl shadow-2xl border border-gray-200 overflow-hidden">
            ...
            <div class="p-6">
                ...
                <form action="{{ route('medecin.store') }}" method="POST" class="space-y-4">
                    ...
                    <div>
                        <label
                            class="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-2">{{ __('Nom complet') }}
                            <span class="text-red-500">*</span></label>
                        <div class="relative">
                            <div
                                class="absolute left-3.5 top-1/2 -translate-y-1/2 w-7 h-7 bg-blue-50 rounded-lg flex items-center justify-center pointer-events-none">
                    ...
                    <div>
                        <label
                            class="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-2">{{ __('Email') }}
                            <span class="text-red-500">*</span></label>
                        <div class="relative">
                            <div
                                class="absolute left-3.5 top-1/2 -translate-y-1/2 w-7 h-7 bg-blue-50 rounded-lg flex items-center justify-center pointer-events-none">
                    ...
                    <div>
                        <label
                            class="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-2">{{ __('Téléphone') }}</label>
                        <div class="relative">
                            <div
                                class="absolute left-3.5 top-1/2 -translate-y-1/2 w-7 h-7 bg-emerald-50 rounded-lg flex items-center justify-center pointer-events-none">
                    ...
                    <div>
                        <label
                            class="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-2">{{ __('Mot de passe') }}
                            <span class="text-red-500">*</span></label>
                        <div class="relative">
                            <div
                                class="absolute left-3.5 top-1/2 -translate-y-1/2 w-7 h-7 bg-purple-50 rounded-lg flex items-center justify-center pointer-events-none">
    ...
    {{-- MODAL — Supprimer --}}
    <div id="modalDelete" class="fixed inset-0 hidden items-center justify-center p-4 bg-black/40 backdrop-blur-sm"
        style="z-index:10000;">
        <div class="relative w-full max-w-md bg-white rounded-2xl shadow-2xl border border-gray-200 overflow-hidden">
            ...
            <div class="p-6">
                ...
                <div class="mb-6 p-4 bg-red-50 rounded-xl border border-red-200">
                    <p class="text-gray-700 text-sm leading-relaxed">
                        {{ __('Êtes-vous sûr de vouloir supprimer ce médecin ?') }}</p>
    ...
    <script>
        ...
        document.querySelectorAll('.action-menu-btn').forEach(btn => {
            btn.addEventListener('click', function (e) {
                ...
                if (activeBtn === btn && !floatingMenu.classList.contains('hidden')) {
                    closeFloatingMenu(); return;
                }

                // Set links
                menuShow.href = this.dataset.showUrl;
                menuEdit.href = this.dataset.editUrl;
                menuDelete.dataset.deleteUrl = this.dataset.deleteUrl;

                // Highlight trigger
                if (activeBtn) activeBtn.classList.remove('bg-gray-100');
                activeBtn = btn;
                ...
                if (rect.bottom + menuH + 6 > vp) top = rect.top - menuH - 6;
                if (left < 8) left = 8;
        ...
        function closeFloatingMenu() {
            ...
            if (activeBtn) { activeBtn.classList.remove('bg-gray-100'); activeBtn = null; }
        }

        document.addEventListener('click', e => { if (!floatingMenu.contains(e.target)) closeFloatingMenu(); });
        ...
        document.getElementById('modalDelete')?.addEventListener('click', e => {
            if (e.target === document.getElementById('modalDelete')) closeDeleteModal();
        });
        ...
        document.getElementById('searchInput')?.addEventListener('keyup', function () {
            ...
            let count = 0;
            document.querySelectorAll('#medecinTable .medecin-row').forEach(row => {
