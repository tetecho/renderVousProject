...
<x-app-layout>
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-gray-500 mt-1">
                            {{ __('Total :') }} <span class="font-bold text-gray-900">{{ $rendezVous->total() }}</span>
                            {{ __('rendez-vous') }}
                        </p>
                    </div>
                </div>
        ...
        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            <div class="flex-1 max-w-2xl">
                <div class="relative group">
                    ...
                    <input type="text" id="searchInput" placeholder="{{ __('Rechercher un rendez-vous...') }}"
            ...
            <div
                ...
                <span>{{ __('sur') }}</span>
                <span class="font-bold text-gray-900" id="totalCount">{{ $rendezVous->total() }}</span>
            </div>
        </div>
            ...
            <div class="text-center py-16 lg:py-20">
                ...
                <p class="text-gray-500 max-w-md mx-auto mb-8">
                    @if($currentUser->isMedecin())
                        {{ __('Aucun rendez-vous programmé pour le moment.') }}
                    @else
                        {{ __('Ajoutez votre premier rendez-vous pour commencer.') }}
                    @endif
                </p>
                @if($canCreate)
                    <button onclick="showAjouteModal()"
            ...
            <div class="space-y-3" id="rendezvousTable">
                @foreach ($rendezVous as $rdv)
                    <div class="rendezvous-row group flex items-center justify-between p-4 bg-white rounded-xl border border-gray-200 hover:border-gray-300 shadow-sm hover:shadow-lg transition-all duration-300 cursor-pointer"
                        onclick="window.location='{{ route('rendezvous.show', $rdv->id) }}'" data-rendezvous-id="{{ $rdv->id }}"
                        ...
                        <div class="flex items-center gap-2 shrink-0 ml-4" onclick="event.stopPropagation()">
                            <button type="button"
                                ...
                                data-id="{{ $rdv->id }}" data-status="{{ $rdv->statut }}" aria-label="{{ __('Actions') }}">
    ...
    @if($canCreate)
        <div id="formAjoute"
            class="fixed inset-0 z-[10000] hidden items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
                ...
                <div class="p-6">
                    ...
                    <form action="{{ route('rendezvous.store') }}" method="POST" class="space-y-4">
                        ...
                        @if($isAdmin)
                            <div>
                                <label
                                    class="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-2">{{ __('Patient') }}
                        ...
                        <div>
                            <label
                                class="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-2">{{ __('Médecin') }}
                                <span class="text-red-500">*</span></label>
                        ...
                        <div>
                            <label
                                class="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-2">{{ __('Service') }}
                                <span class="text-red-500">*</span></label>
                        ...
                        <div>
                            <label
                                class="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-2">{{ __('Date et heure') }}
                                <span class="text-red-500">*</span></label>
                            <div class="relative">
                                <div
            ...
            <div class="p-6">
                ...
                <div class="mb-6 p-4 bg-red-50 rounded-xl border border-red-200">
                    <p class="text-gray-700 text-sm leading-relaxed">
                        {{ __('Êtes-vous sûr de vouloir supprimer ce rendez-vous ?') }}</p>
    ...
    <style>
        ...
        .pagination span {
            @apply flex items-center justify-center min-w-[2.5rem] h-10 px-3 rounded-xl text-sm font-medium transition-all duration-200;
        }

        .pagination .page-link {
            @apply bg-white border border-gray-200 text-gray-700 hover:border-gray-300 hover:shadow-md;
        }

        .pagination .page-item.active .page-link {
            @apply gradient-primary text-white border-transparent shadow-md;
        }
        ...
        #rdvDropdown.rdv-open {
            display: block;
            animation: rdvFade 0.12s ease-out;
        }
        ...
        .rdv-item.rdv-danger:hover {
            background: #fef2f2;
        }

        .rdv-item.rdv-confirm:hover {
            background: #f0fdf4;
        }

        .rdv-item.rdv-cancelitem:hover {
            background: #fffbeb;
        }

        .rdv-item.rdv-disabled {
            opacity: 0.4;
            cursor: not-allowed;
            pointer-events: none;
        }
    ...
    <script>
        ...
        // Attach dropdown event listeners to all action buttons (for both static and dynamic rows)
        function bindActionButtons() {
            document.querySelectorAll('.rdv-action-btn').forEach(btn => {
                ...
                const handler = function (e) {
                    ...
                    currentRdvStatus = this.dataset.status;
                    ...
                    void rdvDrop.offsetWidth;
                    rdvDrop.classList.add('rdv-open');
                };
                btn.addEventListener('click', handler);
                btn._listener = handler; // store for potential removal
            });
        }
        ...
        // Admin confirmation/cancel handlers
        @if($isAdmin)
        ...
        function renderRendezVous(data) {
            const container = document.getElementById('rendezvousTable');
            container.innerHTML = '';
            ...
            if (data.length === 0) {
                container.innerHTML = `
                    <div class="text-center py-12 bg-gray-50 rounded-xl border border-gray-200">
                        ...
                        <p class="text-gray-500">{{ __('Aucun rendez-vous trouvé') }}</p>
                    </div>
                `;
            ...
            data.forEach(rdv => {
                ...
                let badgeClass = '';
                if (rdv.statut === 'confirme') badgeClass = 'bg-green-50 text-green-600 border-green-200';
                else if (rdv.statut === 'annule') badgeClass = 'bg-red-50 text-red-600 border-red-200';
                else badgeClass = 'bg-yellow-50 text-yellow-600 border-yellow-200';
                ...
                const rowHtml = `
                    <div class="rendezvous-row group flex items-center justify-between p-4 bg-white rounded-xl border border-gray-200 hover:border-gray-300 shadow-sm hover:shadow-lg transition-all duration-300 cursor-pointer"
                         onclick="window.location='/rendezvous/${rdv.id}'"
                        ...
                        <div class="flex items-center gap-4 flex-1 min-w-0">
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 mb-1">
                                    <h3 class="text-base font-semibold text-gray-900 truncate">
                                        ${escapeHtml(rdv.patient.name)} → ${escapeHtml(rdv.medecin.name)}
                                    </h3>
                                    <span class="px-2 py-0.5 text-xs font-medium rounded-full border shadow-sm shrink-0 ${badgeClass}">
                                ...
                                <div class="flex flex-wrap items-center gap-3 text-sm text-gray-500">
                                    <div class="flex items-center gap-1.5">
                                        <i class="fas fa-stethoscope text-xs"></i>
                                        <span>${escapeHtml(rdv.service.name)}</span>
                                    </div>
                                    <div class="flex items-center gap-1.5">
                                        <i class="fas fa-calendar-alt text-xs"></i>
            ...
            // Update visible count and hide pagination
            document.getElementById('visibleCount').textContent = data.length;
            if (paginationContainer) paginationContainer.style.display = 'none';
            // Re-attach dropdown listeners to new buttons
            bindActionButtons();
        }

        function resetToOriginal() {
            // Reload the page to restore original paginated view
        ...
        searchInput?.addEventListener('keyup', function () {
            ...
            if (term === '') {
                // Reload page to get back to paginated state
                resetToOriginal();
                return;
            }

            searchTimeout = setTimeout(() => {
                axios.get("{{ route('rendezvous.search') }}", { params: { search: term } })
                    .then(res => {
                        renderRendezVous(res.data);
                    })
                    .catch(err => {
                        console.error(err);
                    });
